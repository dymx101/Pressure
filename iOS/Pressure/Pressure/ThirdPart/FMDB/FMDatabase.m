...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            MLOG(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(db));
            MLOG(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            MLOG(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(db));
            MLOG(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            MLOG(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(db));
            MLOG(@"DB Query: %@", sql);
        }
...
////		else 
////		{
////			ret =[self sdlExecuteUpdate:sql error:outErr withArgumentsInArray:arrayArgs orVAList:args];
////			break;
////		}
////	}	
//	updateLock =nil == updateLock? [[NSLock alloc] init]:updateLock;
//	[updateLock tryLock];
//	ret = [self sdlExecuteUpdate:sql error:outErr withArgumentsInArray:arrayArgs orVAList:args];
//	[updateLock unlock];
//	return ret;
