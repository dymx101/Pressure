...
#import "unistd.h"
#define FMDBQuickCheck(SomeBool) { if (!(SomeBool)) { MLOG(@"Failure on line %d", __LINE__); return 123; } }
...
-(void)testThread1:(FMDatabase*) db;
-(void)testThread2:(FMDatabase*) db;
...
-(void) testThread2:(FMDatabase*) db
{
	...
	int i = 1000;
	while (i++ < 12000) {
        bool bsucc = [db executeUpdate:@"insert into test (a, b, c, d, e) values (?, ?, ?, ?, ?)" ,
         @"hi again'", // look!  I put in a ', and I'm not escaping it!
         [NSString stringWithFormat:@"%d", i],
         @(i),
         [NSDate date],
...
int fmdb_test_main (int argc, const char * argv[]) {
    NSAutoreleasePool * pool = [[NSAutoreleasePool alloc] init];
    testDb * testdb = [[[testDb alloc]init]autorelease];
    // delete the old db.
    ...
    if (![db open]) {
        MLOG(@"Could not open db.");
        [pool release];
        return 0;
    }
	
	FMDatabase* db2 = [FMDatabase databaseWithPath:@"/tmp/tmp.db"];
    if (![db open]) {
        MLOG(@"Could not open db.");
        [pool release];
        return 0;
    }
	//[db setBusyRetryTimeout:10];
    ...
    if ([db hadError]) {
        MLOG(@"Err %d: %@", [db lastErrorCode], [db lastErrorMessage]);
    }
	...
	//[NSThread detachNewThreadSelector:@selector(testThread1:) toTarget:testdb withObject:db];
  //  [NSThread detachNewThreadSelector:@selector(testThread2:) toTarget:testdb withObject:db];
    // do it again, just because
	
	[testdb testThread2:db2];
    ...
    while ([rs next]) {
        ...
        if (!([[rs columnNameForIndex:0] isEqualToString:@"rowid"] &&
            ...
            MLOG(@"WHOA THERE BUDDY, columnNameForIndex ISN'T WORKING!");
            return 7;
        }
    }
    ...
    MLOG(@"Count of changes (should be 1): %d", [db changes]);
    FMDBQuickCheck([db changes] == 1);
    
    int a = [db intForQuery:@"select a from t1 where a = ?", @5];
    if (a != 5) {
        MLOG(@"intForQuery didn't work (a != 5)");
    }
    ...
    MLOG(@"Testing the busy timeout");
    
    BOOL success = [db executeUpdate:@"insert into t1 values (5)"];
    
    if (success) {
        MLOG(@"Whoa- the database didn't stay locked!");
        return 7;
    }
    else {
        MLOG(@"Hurray, our timeout worked");
    }
    ...
    if (!success) {
        MLOG(@"Whoa- the database shouldn't be locked!");
        return 8;
    }
    else {
        MLOG(@"Hurray, we can insert again!");
    }
    ...
    if (![db executeUpdate:@"insert into t2 values (?, ?)", nil, @5]) {
        MLOG(@"UH OH, can't insert a nil value for some reason...");
    }
    ...
    while ([rs next]) {
        ...
        if ([rs2 intForColumnIndex:0] != newVal) {
            MLOG(@"Oh crap, our update didn't work out!");
            return 9;
        }
    ...
    while ([rs next]) {
        ...
        if (!b) {
            MLOG(@"Oh crap, the nil / null inserts didn't work!");
            return 10;
        }
        
        if (a) {
            MLOG(@"Oh crap, the nil / null inserts didn't work (son of error message)!");
            return 11;
        }
        else {
            MLOG(@"HURRAH FOR NSNULL (and nil)!");
        }
    }
    ...
    while ([rs next]) {
        ...
        if (a) {
            MLOG(@"Oh crap, the null date insert didn't work!");
            return 12;
        }
        
        if (!c) {
            MLOG(@"Oh crap, the 0 date insert didn't work!");
            return 12;
        }
        ...
        if (floor(dti) > 0.0) {
            MLOG(@"Date matches didn't really happen... time difference of %f", dti);
            return 13;
        }
    ...
    [db executeUpdate:@"insert into nulltest2 (s, d, i, f, b) values (?, ?, ?, ?, ?)" , @"Hi", safariCompass, @12, @4.4f, @YES];
    ...
    
    {
        ...
        [db executeUpdate:@"insert into testOneHundredTwelvePointTwo values (?, ?)" withArgumentsInArray:@[@"one", @2]];
        [db executeUpdate:@"insert into testOneHundredTwelvePointTwo values (?, ?)" withArgumentsInArray:@[@"one", @3]];
        
        
        rs = [db executeQuery:@"select * from testOneHundredTwelvePointTwo where b > ?" withArgumentsInArray:@[@1]];
    ...
    
    {
        ...
        FMDBQuickCheck([db executeUpdate:@"drop table t4;" withArgumentsInArray:@[]]);
        FMDBQuickCheck([db executeUpdate:@"create table t4 (a text, b text)" withArgumentsInArray:@[]]);
        FMDBQuickCheck(([db executeUpdate:@"insert into t4 (a, b) values (?, ?)" withArgumentsInArray:[NSArray arrayWithObjects:@"one", @"two", nil]]));
        
        rs = [db executeQuery:@"select t4.a as 't4.a', t4.b from t4;" withArgumentsInArray:@[]];
    ...
    MLOG(@"That was version %@ of sqlite", [FMDatabase sqliteLibVersion]);
