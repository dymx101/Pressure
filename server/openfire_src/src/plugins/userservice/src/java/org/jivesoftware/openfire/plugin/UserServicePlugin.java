...
public class UserServicePlugin implements Plugin, PropertyEventListener {
	...
	public void initializePlugin(PluginManager manager, File pluginDirectory) {
		...
		enabled = JiveGlobals.getBooleanProperty("plugin.userservice.enabled",
				false);

		// Get the list of IP addresses that can use this service. An empty list
		// means that this filter is disabled.
		allowedIPs = StringUtils.stringToCollection(JiveGlobals.getProperty(
	...
	public void createUser(String username, String password, String name,
		...
		if (groupNames != null) {
			...
			while (tkn.hasMoreTokens()) {
				try {
					groups.add(GroupManager.getInstance().getGroup(
							tkn.nextToken()));
	...
	public void updateUser(String username, String password, String name,
			String email, String groupNames) throws UserNotFoundException {
		User user = getUser(username);
		if (password != null)
		...
		if (groupNames != null) {
			...
			while (tkn.hasMoreTokens()) {
				try {
					newGroups.add(GroupManager.getInstance().getGroup(
							tkn.nextToken()));
				} catch (GroupNotFoundException e) {
					// Ignore this group
				}
			...
			Collection<Group> existingGroups = GroupManager.getInstance()
					.getGroups(user);
			...
			Collection<Group> groupsToDelete = new ArrayList<Group>(
					existingGroups);
	 ...
	 */
	public void addRosterItem(String username, String itemJID, String itemName,
			String subscription, String groupNames)
		...
		if (r != null) {
			...
			RosterItem ri = r
					.createRosterItem(j, itemName, groups, false, true);
			if (subscription == null) {
				subscription = "0";
			}
			ri.setSubStatus(RosterItem.SubType.getTypeFromInt(Integer
					.parseInt(subscription)));
	 ...
	 *             if roster item cannot be added to a shared group.
	 */
	public void updateRosterItem(String username, String itemJID,
			String itemName, String subscription, String groupNames)
	...
	public void setAllowedIPs(Collection<String> allowedIPs) {
		JiveGlobals.setProperty("plugin.userservice.allowedIPs",
				StringUtils.collectionToString(allowedIPs));
	...
	public void setEnabled(boolean enabled) {
		...
		JiveGlobals.setProperty("plugin.userservice.enabled", enabled ? "true"
				: "false");
	}
	...
	public void propertySet(String property, Map<String, Object> params) {
		if (property.equals("plugin.userservice.secret")) {
			...
			this.allowedIPs = StringUtils.stringToCollection((String) params
					.get("value"));
		}
	}
